...
| -------- | -------------------------------------- |
| Users at launch? In 6 months? | The case study describes TutorMeAI at large scale (districts, hundreds of thousands of daily users). The **MVP architecture** is scoped to prove the plugin contract (Chess vertical, in-repo apps, single-region Railway). Explicit numeric SLOs for launch vs. six months are **not locked** in the architecture doc; deployment assumes **single-region** unless product requires otherwise. |
...
| Pay-per-use acceptable or need fixed costs? | **Not specified**; managed PostgreSQL and Railway imply operational cost without a fixed ceiling in the doc. |
...
| Question | Pre-search answer (from architecture) |
| -------- | -------------------------------------- |
| How will you isolate third-party app code? | **Sandboxed iframes**, **restrictive `sandbox` flags**, **distinct allowed origins** where practical, **CSP** allowing only **known app origins** for MVP, **host–iframe via `postMessage` only**. |
...
| Languages/frameworks you know well? | Stack hints: **Next.js-class** host, **PostgreSQL**, **Railway** deployment; exact language list not mandated in the arch doc. |
| Experience with iframe/postMessage communication? | Architecture **requires** host–iframe **`postMessage`** with **typed, versioned messages** and host validation—this is a **core skill assumption** for implementation. |
...
| Message passing protocol (postMessage, custom events, WebSocket)? | **WebSocket**: host ↔ server (streaming, run status, app commands, patches, events). **`postMessage` only**: host ↔ iframe. |
| How does the chatbot discover available tools at runtime? | **Per turn**, server assembles **only eligible** tools (active app, teacher allowlist, auth readiness, conversation mode, flags). **Full global registry is not injected** into every prompt. |
...
| How will dynamic tool schemas be injected into the system prompt? | **Run orchestrator** assembles context: transcript, tool messages, **compact server-validated app snapshot**, run status, **eligible tool schemas**—with **token budget** and pruning rules. |
...
| Streaming responses while waiting for tool results? | **WebSocket** carries **assistant token streaming** alongside **run status** and app lifecycle; run state machine includes states like **`invoking_tool`**, **`awaiting_app_ack`**, **`awaiting_user_action`** so the UX can reflect waiting vs streaming. |

### 8. Real-Time Communication

| Question | Pre-search answer (from architecture) |
| -------- | -------------------------------------- |
| WebSocket vs SSE vs polling for chat? | **Preferred MVP: WebSocket** for one bidirectional, ordered session channel (vs split SSE + HTTP). |
| Separate channel for app-to-platform communication? | **No separate MVP channel** for app events vs chat: **one persistent session channel** for assistant streaming, run status, app patches, tool progress, app events. **Iframe still uses postMessage to host**, which bridges to the server over WS. |
| How do you handle bidirectional state updates? | Server → host: commands/patches; host → server: validated app events; server updates persistence and may emit further commands or continue the run. |
...
| Question | Pre-search answer (from architecture) |
| -------- | -------------------------------------- |
| Where does chat state live? App state? Session state? | **Persistent store**: users, sessions, conversations, messages, app sessions, OAuth, execution records. **Domain state** (canonical, versioned) vs **view state** (ephemeral UI). |
...
| Question | Pre-search answer (from architecture) |
| -------- | -------------------------------------- |
| Platform auth vs per-app auth? | **Platform**: server-verified session (e.g. NextAuth-class). **Per-app**: internal / external public / **external authenticated** (OAuth). |
...
| CSP headers for embedded content? | **CSP** allows **only known MVP app origins**; **fixed allowlist**. |
| Preventing apps from accessing parent DOM? | **Iframe boundary** + **origin isolation**; host validates **postMessage** origin; no same-origin plugin embedding for MVP. |
...
| Question | Pre-search answer (from architecture) |
| -------- | -------------------------------------- |
| How do you test the plugin interface in isolation? | **Typed protocol** (envelope fields, message types) supports **contract tests** between host, server, and iframe without a full LLM. |
...
This pre-search is derived from the locked decisions and rationale in [`docs/architecture.md`](architecture.md) (ChatBridge Architecture), aligned with the TutorMeAI case study and assignment framing in [`docs/prd.md`](prd.md).
